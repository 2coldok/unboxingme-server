...
export async function validateChallengeablePandora(req, res, next) {
  try {
    ...
    if (!pandora) {
      const data = mInitialRiddle(null, 'INACTIVE');
      return failResponse(res, 404, data, '판도라를 찾을 수 없음 | 판도라 비활성화 | 판도라 solver 존재');
    }

    // 판도라 생성자가 수수께끼를 도전하려고 할 경우
    const googleId = req.googleId;
    if (pandora.maker === googleId) {
      const data = mInitialRiddle(null, 'MINE');
      return failResponse(res, 403, data, '자신의 수수께끼는 도전할 수 없습니다.');
    }

    // 안전을 위해 maker 제거하고 next
    const { maker, ...rest } = pandora;
