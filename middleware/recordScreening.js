...
export async function recordScreening(req, res, next) {
  ...
  const record = await recordDB.findRecord(userId, pandoraId);

  if (record !== null && record.unboxing) {
    return res.status(403).json({
      message: '이미 unboxing한 판도라의 상자입니다.'
    });
  }
  
  if (record === null) {
    console.log('????');
    const record = await recordDB.create(userId, pandoraId);
    req.record = record;
  ...
  if (record && isAccessRestrictedByPenalty(record.restrictedUntil)) {
    ...
    return res.status(403).json({
      ...
      restrictedUntil: moment(record.restrictedUntil).format('YYYY-MM-DD HH:mm:ss'),
    });
  }
